...
export function createPuppy(source) {
 ...
 source.traverse(o=>{if(!o.isMesh)return;
  ...
  for(let i=0;i<p.count;i++){
   ...
   else if(headWeight>.01){const earWeight=THREE.MathUtils.smoothstep(Math.abs(x),.085,.135)*(1-THREE.MathUtils.smoothstep(y,.73,.79))*THREE.MathUtils.smoothstep(y,.51,.59)*(1-THREE.MathUtils.smoothstep(z,.27,.34));indices.push(bones.indexOf(spine),bones.indexOf(head),bones.indexOf(ears[x<0?0:1]),0);weights.push(1-headWeight,headWeight*(1-earWeight),headWeight*earWeight,0);}
   else{indices.push(bones.indexOf(z>.02?chest:spine),1+closest*3,2+closest*3,3+closest*3);weights.push(1-legWeight,legWeight*(1-kneeWeight),legWeight*kneeWeight*(1-pawWeight),legWeight*kneeWeight*pawWeight);}
 ...
 const motion=createPuppyMotion({group,root,legs,spine,head,tail,tailTip,chest,ears});
