...
export function createPuppy(source) {
 ...
 for(const [side,front] of [[-1,1],[1,1],[-1,-1],[1,-1]]){
  ...
  limbCenters.push(sum.clone());const upper=new THREE.Bone(),lower=new THREE.Bone();upper.name=`${front>0?'front':'hind'}_${side>0?'right':'left'}`;lower.name=upper.name+'_knee';upper.position.set(sum.x,.44,sum.z);lower.position.set(0,-.235,0);root.add(upper);upper.add(lower);bones.push(upper,lower);const paw=new THREE.Bone();paw.name=upper.name+'_paw';paw.position.y=-.205;lower.add(paw);bones.push(paw);legs.push({upper,lower,paw,length1:.235,length2:.205,front:front>0,rest:new THREE.Vector3(sum.x,0,sum.z)});
 }
 ...
 source.traverse(o=>{if(!o.isMesh)return;
  ...
  for(let i=0;i<p.count;i++){
   ...
   if(tailWeight>.01){indices.push(0,bones.indexOf(tail),0,0);weights.push(1-tailWeight,tailWeight,0,0);}
   else if(headWeight>.01){indices.push(0,bones.indexOf(head),0,0);weights.push(1-headWeight,headWeight,0,0);}
   else{indices.push(0,1+closest*3,2+closest*3,3+closest*3);weights.push(1-legWeight,legWeight*(1-kneeWeight),legWeight*kneeWeight*(1-pawWeight),legWeight*kneeWeight*pawWeight);}
