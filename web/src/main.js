...
function runPuppy(value){if(!puppyRig)return;puppyRig.setRunning(value,elapsed);renderer.shadowMap.needsUpdate=true;if(value)setMotion(true);$('dog-run').textContent=value?'让它歇一会':'让小狗跑起来';$('dog-run').setAttribute('aria-pressed',String(value));if(!value&&state.selected==='puppy')focus('puppy');announce(value?'小狗沿着溪边小路跑起来了':'小狗慢慢停下来休息');}
